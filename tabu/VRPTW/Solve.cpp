...
{
	ofstream out;
	out.open("C:\\Users\\MiaoAYao\\Desktop\\result1.txt", std::ios::app);
	...
	out << temp_s->routeSet.size() << "  " << 0.7 * temp_s->allLength + 0.3 * temp_s->allTime;
	out << endl;
	...
	if (tmp != n_s->routeSet[r]->tail) //这个节点不为仓库节点时
	{
		...
		while (tmp != n_s->routeSet[r]->tail)
		{
			...
			if (n_s->routeSet[r]->isFeasibleGC(node_i->num, node_i->d_t, node_j, data))
			{
				//如果可行
				//判断是否在禁忌表中
				vector<double>* tmp_n = new vector<double>();
				tmp_n->push_back(1);
				tmp_n->push_back(node_i->num);
				tmp_n->push_back(node_j->num);
				tmp_n->push_back(r);
				if (!isTabu(tmp_n))
				{
					//不在禁忌表中
...
bool Solve::isTabu(vector<double>* n)
{
...
void Solve::updateTabuList(int iteration)
{
	for (int i = 0; i < tabu_list.size(); ++i)
	{
...
void Solve::addToTabuList(int iteration, vector<double>* n)
{
...
void Solve::tabuSearch()
{
	...
	for (int i = 0; i < maxIteration; ++i)
	{
		...
		if (g_best_neighbour != -1)
		{
			...
			reverseOperate(&r_n, &best_neighbour, n_s);
			addToTabuList(i, r_n);
